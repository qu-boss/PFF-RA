...
from utils.general import bbox_iou, bbox_mpdiou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy
...
class ComputeLossOTA:
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            #------------------------------------------------------------------------------------
            # top_k, _ = torch.topk(pair_wise_iou, min(10, pair_wise_iou.shape[1]), dim=1)
            # layers = torch.split(top_k, 1, dim=0)
            # dynamic = []
            # for j, layer in enumerate(layers):
            #     # if (txywh[j, 2] * txywh[j, 3]).int() <= 350: # 3600 = 1% math.sqrt((txywh[j, 2] * txywh[j, 3]) / (imgs[..., -1].float() * imgs[..., -1].float()))
            #     if math.sqrt((txywh[j, 2] * txywh[j, 3]) / (640 * 640)) < 0.03:
            #         ks = max(1, layer.size(1))
            #         dynamic.append(ks)
            #         # if layer.sum(1).int() < 2:
            #         #     ks = max(1, layer.size(1))
            #         #     dynamic.append(ks)
            #         # else:
            #         #     ks = layer.sum(1).int()
            #         #     dynamic.append(ks)
            #     else:
            #         ks = max(1, layer.sum(1).int())
            #         dynamic.append(ks)
            # dynamic_ks = torch.tensor(dynamic)

            # -----------------------------------------------------------
            # pair_wise_iou = []
            # for i, tbox in enumerate(txywh):
            #     if math.sqrt((tbox[2] * tbox[3]) / (640 * 640)) < 0.03:
            #         txywh = torch.unsqueeze(tbox, 0)
            #         exp = wasserstein_loss(pxywhs, txywh)
            #         e = torch.unsqueeze(exp, 0)
            #         pair_wise_iou.append(e)
            #         # pair_wise_iou_loss.append(-torch.log(pair_wise_iou + 1e-8))
            #     else:
            #         txyxy = torch.unsqueeze(tbox, 0)
            #         exp = xywh2xyxy(txyxy)
            #         pair_wise_iou.append(box_iou(exp, pxyxys))
            #         # pair_wise_iou_loss.append(-torch.log(pair_wise_iou + 1e-8))
            # pair_wise_iou = torch.stack(pair_wise_iou, dim=0)
            # pair_wise_iou = pair_wise_iou.squeeze(1)
            # pair_wise_iou_loss = -torch.log(pair_wise_iou + 1e-8)
            # top_k, _ = torch.topk(pair_wise_iou, min(10, pair_wise_iou.shape[1]), dim=1)
            # dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)
            # -----------------------------------------------------------
            ...
            #     pair_wise_iou.append(e)
            #
            # pair_wise_iou = torch.cat(pair_wise_iou,dim=0)
            #
            # # pair_wise_iou_loss = -torch.log(pair_wise_iou + 1e-8)
            # pair_wise_iou_loss = (1.0 - pair_wise_iou).mean()
            # top_k, _ = torch.topk(pair_wise_iou, min(10, pair_wise_iou.shape[1]), dim=1)
            # dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)
            #------------------------------------------------------------
...
#       返回构建好的匹配关系，包括匹配的图像索引、锚框索引、   网格索引   、目标框信息和锚框信息。
